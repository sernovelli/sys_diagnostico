 ...
 */
 ...
 class DesativaArquivosModel extends PersistModelAbstract {
...
// 	
	 ...
	 */
	public function listaArquivosVencimento() {
		
		$vetor = array();
		
		$query = "SELECT * 
				  FROM arquivo 
				 WHERE arquivo.dataVence = '".$this->hoje."'
	 ...
	 */
	  ...
	  */
	  public function desativaArquivo($idArquivo) {
		
		$query = "UPDATE arquivo
				   SET arquivoStatus = '2'
				 WHERE arquivo.pk_arquivo = ".$idArquivo."
				   AND arquivo.arquivoStatus = '1'
				   AND arquivo.dataVence = '".$this->hoje."'";
