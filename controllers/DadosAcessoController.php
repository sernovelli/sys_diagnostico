...
// 		
	...
	public function manterAction() {
		...
		if (count($_POST) > 0) {
			...
			$objeto->setLogin(DataFilter::cleanString($_POST['login']));
			$objeto->setSenha(DataFilter::cleanString($_POST['senha']));
			...
			if ($objeto->save() == "") {
				...
				Application::redirect('?controle='.$this->controle.'&acao=manter&id='.$id.'&msg=15');
			} else {
				// Erro na operação
				Application::redirect('?controle='.$this->controle.'&acao=manter&id='.$id.'&msg=2');
			}
		}
